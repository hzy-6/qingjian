...
title: 从第一次输入开始
order: 0
description: 青简输入法使用文档（macOS 测试版）：安装、按键、译词与生词、云联想、词库、数据位置与卸载。
---

青简是一个拼音输入法。输入时，候选词旁边附带一条所学语言的译词，在输入的同时记住一个词。
它首先是一个完整的输入法：整句输入、简拼、拼写纠错、模糊音、双拼、英文模式，全部在本机完成，不上传数据。

macOS 版目前为测试版，见 [按键与快捷键](getting-started/keys.md)。
...
- **全部在本机。** 拼音转换、词库、学习、释义均不联网；云联想是可选的补充，数据发送给用户自行填写的服务商，不经过青简。
