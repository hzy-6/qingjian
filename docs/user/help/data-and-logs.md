...
「~/Library/Application Support/Qingjian/」（访达中按 `⇧⌘G` 输入此路径）。
...
（bundle identifier）。另有几类事件各记一行：组句中退格后重打了不同的键、组句之外直接打出的标点与回车、
...
「~/Library/Logs/Qingjian/」，文件名「qingjian.log.日期」。
