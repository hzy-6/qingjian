...
青简随包带一个几百 MB 的小语言模型（Qwen，量化版），组句时在本机给整句候选重新排一次序。全程离线，不联网、不发送任何内容。
...
用户数据目录的 `model/` 下放自己的模型，重启输入法后优先于随包模型使用。有两类，优先级从高到低：

1. **`.gguf`**（Qwen / llama.cpp 格式）：随包模型就是这个格式；放你自己的量化文件进去会优先生用。目录里有多份时按文件名取排最前的那份。
2. **`.qjm`**（旧版字级模型，训练仓库直接导出的 `model.safetensors`、`config.json`、`vocab.json` 三个文件也认）：只在没有任何 `.gguf` 时才会用到——包括随包里的。想用回旧版模型，把随包与用户目录里的 `.gguf` 都移走即可。
