# Qwen 接管整句重打分:0.8B 起步、2B 定稿(2026-09-19 ~ 09-20)

## 做了什么

- 换模型:随包神经模型从 102M 字级 Transformer(uer/gpt2-chinese-cluecorpussmall 转制,`model.qjm`)换成
  **Qwen3.5-0.8B-Q8_0 GGUF**(774 MB,Apache-2.0,与 GPL-3.0 兼容),放 `data/model/`,`bundle.sh` 打进 `Resources/model/`。
  随包只带 GGUF。(0.8B 时代的 `.qjm` 兜底与 `qingjian-neural` crate 已于 a023f2a 移除——2B 终局定稿后不再需要对照基线。)
- 新 crate `qingjian-qwen`:llama.cpp 的 Rust 绑定(`llama-cpp-2` 0.1.156,feature `runtime` 门控,默认空壳、CI 不拉 C++ 依赖),
  `QwenScorer` 实现 `SentenceScorer`,给「前文 + 整句」按 BPE token 累加 log 概率,Metal 加速,双向上下文(`score_with_after`)。
- Engine 三个新缺省:`NEURAL_GATE` = 2(个人证据保护闸,见下)、`RESCORE_CONTEXT_CHARS` = 128(原 64)、
  Qwen 的修正建议 `max_adjustment` = 30 nat(走模型建议机制,壳不传参数即最优)。λ 仍是 `NEURAL_WEIGHT` = 0.5。
- CLI:`--qwen <gguf>`(与 `--neural` 互斥,共用 `--neural-*` 参数)、`--tune gate=倍率`。

## 为什么要换

自训模型在 wiki+LCCC(12.7 亿 token)量级上 23M→36M→102M 收益封顶,剩余 miss 全是搭配判别
(的/得、再/在、编程/变成、带货/大火)。Qwen3.5 预训练语料大 3~4 个数量级,正是这类判断;
对 eval-miss-closeout 里**小模型时代的 22 条 miss**(λ1.0 口径)它翻正 9 条。注意口径:对 102M@λ0.5 的
逐句对账是**翻正 3 条(吓倒、直播带货、他感冒了)、翻坏 1 条(他不仅→它),净 +2 句;139 句上这个差距
不显著**(discordant 4 对,McNemar p≈0.63,1 句=0.7pp)。换模型的依据是多方证据的合力:搭配类 miss 在
λ1.0 下全翻正(探针逐句实测)、回放词打平、延迟更低、以及 102M 的字级预训练上限已经到了——不是单看 139 句的 +1.4pp。

## 数字

139 句盲评(`data/eval/baseline-eval.txt` + IT/成语词库,冷启动):bigram 71.9% → 102M 87.8% → **Qwen 89.2%**(字准 98.9%,查询平均 72 ms,比 102M 的 102 ms 快——llama.cpp 融合核对比 candle 小算子调度)。

冻结日志回放(2026-09-19 快照,1678 词 / 535 句,日志在实时增长、对比必须先 `cp` 冻结):

| 配置(都是 Qwen λ0.5) | 词 | 整句 |
|---|---|---|
| 102M(换前) | 89.5%(1502) | **87.9%**(470) |
| 无闸 ctx64 | 89.6%* | 86.8%* |
| gate2 ctx64 | 89.4%(1500) | 87.1%(466) |
| gate3 ctx64 | 89.3% | 87.3% |
| **gate2 ctx128(新缺省)** | **89.5%(1502)** | 87.5%(468) |
| gate3 ctx128 | 89.5% | 87.1% |

(*为略早的快照,只看趋势。)λ≥1(引擎钳到 1)盲评封顶 89.9% 但回放整句崩到 84.4——回放的「正确答案」是历史提交,
多半是当时引擎自己的输出,神经压过个人证据就翻案;这正是 eval-miss-closeout 口径警示写的 then/now 偏差。

## 个人证据保护闸

`rescore_paths` 里,挑战者要翻掉老排名靠前的路径(守成者)时,要求
`神经修正差 ≥ gate × 守成者个人证据优势`,不够就把挑战者压回平手(稳定排序让守成者留前)。
个人证据 = `路径分 − 静态分`(个人 n-gram、用户加分、代价)。eval 冷启动没有个人数据,闸零影响;回放整句 86.8 → 87.1(gate2)。
gate≥3 开始挡掉词的翻正(词 89.4 → 89.3),2 是整句/词的平衡点。

逐句对账(gate2,Qwen 与 102M 的整句 miss 差集):Qwen 独有 miss 7 条里 5 条是用户口癖
(恩恩、第二个、都要上、那一类、把过来——用户当年就这么选的);Qwen 独有翻正 6 条(不用呢、词与、分析一起、两哈、使用护照、要通过)
全是 102M 的真实错误。**语言判断全面占优,剩 2 句的差距是个人习语记忆,由学习系统追,不归通用模型管。**

## llama.cpp 的坑(Qwen3.5 是注意力 + SSM 混合架构,`qwen35`)

- `seq_cp` 只支持整段拷贝,且统一 KV 缓存多 stream 时会整块拷贝 KV buffer(一次几百 MB 的 GPU 拷贝,还踩 `invalid token[0] = -1` 的断言)。
- `seq_rm` 中间位置回卷受 SSM 逐 token 快照数(`n_rs_seq`)限制,默认不够;调大则 SSM 状态张量按 (1+n) 倍膨胀,不划算。
- 结论:**每条候选拼上前文各自成一个序列、一次 decode 打一批、打完 `clear_kv_cache`**(整段重算,前文几十个 token 在 Metal 上很便宜)。
  `clear_kv_cache` 才会重置循环状态的尾指针,`seq_rm(-1)` 不会(悬空尾指针会在下次 decode 的 `GGML_ASSERT` 里炸)。
- `dec_start_token_id` 对 qwen35 是 -1(`LLAMA_TOKEN_NULL`),空前文要退到 BOS / EOS 给候选首 token 一个条件分布。
- 超过 512 token 的 batch 被切 ubatch,而混合架构要求一条序列的 token 不跨 ubatch:按 `CHUNK_TOKENS` = 480 预算切批。

## 第二轮:多智能体对抗调优(2026-09-19 下午)

四个智能体(实证探针/静态分析/架构设计/红队)交叉后的改动与结论。**正式尺子换冻结集** `data/eval/sentences-frozen.tsv`
(`--eval-save` 三列格式,拼音不再随词库补频漂移——红队指出原始文本评测会随词库共同进化,历史数字不可复现):

- **margin 8 → 9**(`NEURAL_MARGIN`):k=16 下「银杏叶」这类深名次路径 9 nat 就进池,9-12 同平台、9 最省,回放逐条不变。
- **束宽 8 → 10**(`BEAM_WIDTH`):探针定位的「终点节点束剪」——好结尾但静态分不够的节点(「微风|轻拂」在 4 字句
  被高频单字组合挤出前 8)活不到终点,神经分再对也没用。10 让它活下来,盲评 +0.7(轻拂 翻正)、回放词 +1、
  回放整句不变,**102M 同样受益**(87.8→88.5,结构公平)。12 与 10 同分,不要。
- **`--neural-paths` 可配**(`Engine::set_neural_paths`,缺省 16):诊断证明 k 对本清单增益≈0——剩余 miss 是
  **结构问题不是 k 问题**:双分歧盲区(「不曾+起舞」两处同时偏离静态最优,分歧链每节点只换一档 back2,无法复活)、
  终点束剪(束宽 10 已治)、或模型判断。crate-notes 里「= 8」的旧文档已纠正。
- **格子宽度放宽到 10:试过,撤销**(`convert_paths` 的 `span_width` 参数与缓存键宽度支持保留作实验口):
  净伤害 1.4 个点——低频路径把 k=16 的重排池挤爆,厨师/火候、直播带货 反被顶出去。
- λ 0.6/0.75/1.0 全部 ≤ 同分且字准更低;λ1.0 修搭配(的/得/地/夸奖他,探针逐句实测)翻代词,**零和**。
- 红队命中的口径修正:「翻正 9 条」是小模型时代 miss 表的 λ1.0 口径;对 102M@λ0.5 的逐句对账是净 +2 句
  (139 句上不显著,McNemar p≈0.63);cap30 是盲评扫出的缺省,不是训练侧建议。

## 最终数字(冻结集 + 冻结回放快照 1678 词 / 535 句)

| 配置 | 盲评首选 | 盲评字准 | 回放词 | 回放整句 |
|---|---|---|---|---|
| 无神经 | 71.9% | 95.8% | 88.5% | 86.6% |
| 102M(被换下的) | 88.5% | 98.8% | 89.5% | **87.9%** |
| **Qwen(全缺省)** | **90.6%** | **99.1%** | **89.6%** | 87.5% |

剩余 13 条 miss:6 条无上下文代词(她/他/它,信息论上限)+ 的/得/是×3(λ1.0 可修但翻代词,零和)+
再/在、会议室(bigram 语料,复开条件是 lm.qj 重统计)+ 数据分析师(终点不在束内,原因待查)+ 不曾起舞(双分歧盲区,
要加深分歧组合,结构改动留观)。下一档提升的出路:评测集扩容混域(closeout 行动项)、lm.qj 语料重统计、底座换代。

## 第三轮:lm.qj 语料重统计(2026-09-19 晚,最终采用 fill-in 混合)

管线全通:hf-mirror 拉 维基20231101(2.4G)+LCCC(0.8G)+知乎KOL(2.0G)→ `parquet_to_text` → `bigram`(1.9 亿句,
800 万 bigram)→ `pack lm`。四种方案 × 两把尺:

| lm | 盲评 Qwen | 盲评静态 | 回放词 | 回放整句 |
|---|---|---|---|---|
| 旧(维基+LCCC) | 90.6 / 99.1 | 71.9 / 95.8 | **89.6** | **87.5** |
| 全新替换(知乎全量) | 90.6 | 83.5 | 89.1 | 84.9 |
| 全新替换(维基×2+知乎半) | 92.1 | 82.7 | 88.0 | 83.9 |
| **fill-in 混合(采用)** | **92.1 / 99.3** | 79.9 / 97.4 | 89.4(−3) | 86.4(−6) |

**全新替换必损回放 14-19 句**(前五不动、首选换位——静态先验一换用户历史选择全体挪位)。**fill-in**(`tools/corpus/lm_fillin.py`):
先从 lm.qj 容器把旧计数导成 TSV(容器布局:头 32B/节项 24B/WordEntry 12B/Successor 8B/CSR),再把新语料的计数
**只填旧表没有的条目**(按新旧总计数比 0.31 缩放、封顶在前词旧计数一半),已有条目一律不动——熟悉排序保留,
覆盖补上(的→会议室 95、数据→分析师 1071、不要→再 1332)。

盲评 90.6 → **92.1**(13 条 miss 修好 4:数据分析师、他的普通话说得、他不仅学习好、不曾起舞;新破 2:雨下的、网络连接),
回放 −6 句(换位型,前五 97.9% 不动,个人 n-gram 会随使用回填)、词 −3。回退一条命令:`cp /tmp/lm-old.qj data/generated/lm.qj && bundle.sh --install`。

教训:语域配比敏感(口语压坏正式句,维基加权伤口语回放);会议室/再犹豫 的 fill 计数被缩放压得太低没翻动,
要它们得提高 fill 上限(会同步放大回放位移,未做);评测集扩容前,139 句上 ±2 句的差异当噪声读。
知乎内容许可未声明(知乎条款默认 CC-BY-NC),fill-in 只取计数统计且 lm 随包不分发源文本,如将来对外分发需重估。
语料与各版 TSV 留在 data/(gitignore,约 15G)。

## 第四轮:多模态裁剪与剩余项(2026-09-20)

### GGUF 词表裁剪(已随包)

解剖 774MB 的 GGUF:320 个张量里**没有视觉/音频塔**(unsloth 出品即纯文本版,多模态在独立的 mmproj 文件、从未下载);
真正的大头是 **248,320 行的词嵌入(254M 参数,占 33.8%)**,其中 96,677 行是泰/阿/韩文碎片与 `<|image_pad|>`/`tts_*`
等中文输入法永远打不出的 token。`tools/corpus/trim_gguf_vocab.py` 按 Q8_0 行块(34B×32参数)切除这些行、
重写词表/类型/eos 元数据与张量偏移(GGUF 的 offset 相对数据区起点;dims 是 [ne0=hidden, ne1=vocab]——
两处都踩过坑,工具里都有注释)。**774MB → 672MB(−102MB),盲评逐句一致(92.1/99.3),延迟 77→63ms**
(logsumexp 少算 9.7 万行),回放 −2 句(softmax 重归一的边缘翻转)。随包优先挑 `*-text.gguf`。

### 静态句首上下文(code-tracer 的「上下文盲」假设:证伪为跨句、证真为句内)

`convert_paths` 增加 `start_context` 参数、引擎喂 `chain.context()`(连打时第二段组句的**句内**左文进静态转移,
以前丢掉,这是真收益);`Engine::seed_chain` 可外部种链。但评测侧实验(895 句混域尺,成对句互为上下文):
喂跨句左词给静态 **−0.5 个点**——bigram 统计按句切分,跨句左词是分布外输入,`Context::START` 本来就是正确的
边界先验。评测因此不种链(上文只喂 Qwen);引擎侧贯通保留。

### 大尺(895 句混域,知乎/维基/LCCC/文档四域成对抽句)

Qwen+全套 60.2%(字准 90.5)、纯静态 52.2%——比 139 句集难得多、也更接近真实水平;139 尺的 89-92% 偏乐观。
新尺冻结在 `data/eval/corpus-ctx.tsv`,以后调参以它为准。fill-in 全额上限试过:两尺零变化(会议室/再犹豫
卡在对手计数,不在上限),维持半量。

## 验收

```bash
cargo run --release -p qingjian-cli --features qwen -- \
  --eval-text data/eval/sentences-frozen.tsv \
  --extra-dict data/generated/dicts/idioms.qj --extra-dict data/generated/dicts/it_computing.qj \
  --qwen data/model/Qwen3.5-2B-UD-Q4_K_XL-text.gguf  # 盲评 93.5% / 99.3%(2B + λ0.75 + paths8)
cargo run --release -p qingjian-cli --features qwen -- \
  --eval-text data/eval/corpus-ctx.tsv \
  --extra-dict data/generated/dicts/idioms.qj --extra-dict data/generated/dicts/it_computing.qj \
  --qwen data/model/Qwen3.5-2B-UD-Q4_K_XL-text.gguf  # 大尺 895 句:61.6% / 90.9%
cargo run --release -p qingjian-cli --features qwen -- \
  --replay <冻结的 input-log 快照> --qwen data/model/Qwen3.5-2B-UD-Q4_K_XL-text.gguf
```

裸 `--qwen` 不带任何调参 flag 即出厂配置(λ 0.75 / cap 30 / gate 2 / margin 9 / 束宽 10 / paths 8 / ctx 128 全是缺省)。
工具:`tools/corpus/lm_fillin.py`(lm 计数导出+fill-in 混合)、`tools/corpus/trim_gguf_vocab.py`(GGUF 词表裁剪)。

## 第五轮:Qwen3.5-2B 对局 0.8B(2026-09-20,采纳 2B)

用户提出试 "qwen3.5-2b-mlx-4bit"。MLX 是苹果的另一套推理格式,本仓栈是 llama.cpp(GGUF),等价物取
unsloth 的 **UD-Q4_K_XL**(动态混合量化,同权重不同容器)。三智能体(量化调研 / 对局设计 / 预注册裁判)+ 主会话实测:

- 2B 同为 qwen35 混合架构、同 248k 词表(eos=248046 与 0.8B 一致)——词表裁剪工具直接可用;
  **裁剪工具修了通用性 bug:行宽改按实测(张量总字节 ÷ 词表行数),不再假设 Q8_0 布局**(UD 混合量化的
  embd 非 Q8,旧算法产出损坏文件)。
- 裸参数对局(同缺省 λ0.5):139 尺同分 92.1/99.3;**895 主尺 61.5/91.0 vs 0.8B 60.2/90.5**,
  全量 miss 对账 2B 独有修好 17 / 翻坏 6,**McNemar 精确 p=0.035(显著)**;回放同分 89.3/86.0(零回归)。
- 对称轻扫:cap40 与 cap30 逐数同分(2B 对 cap 不敏感);**λ0.75 在 895 上 61.8/91.2**,按纪律用
  139+回放独立确认:**139 尺 94.2/99.4(历史新高,+2.1)**,回放词 89.5(1502,+3)、整句 85.4(−3,换位噪声带)。
- **采纳(裁判预注册终局):随包换 Qwen3.5-2B-UD-Q4_K_XL-text(1175MB),λ 缺省 0.5→0.75、RESCORE_PATHS 16→8**
  (0.8B 移 data/model-archive 留档,用回它建议 CLI 覆盖 --neural-weight 0.5)。
- **裁判两颗螺丝的闭环**:①最慢延迟——paths16 时 139 尺预热后稳定 318-320ms,破一票否决线 300ms,按预注册救火分支
  `--neural-paths 8` 复测:最慢 218ms(线内)、平均 72ms,139 = 93.5/99.3、895 = 61.6/90.9(比 paths16 只差 0.2 个点,
  k 不敏感的诊断再次验证),出厂定 paths=8;②四域分账——23 句 discordant 逐句回标原文来源:修好 17 句 = 知乎 9 + 维基 4 +
  docs 3 + lccc 1,翻坏 6 句 = 知乎 3 + docs 3,四域净差 知乎 +6 / 维基 +4 / lccc +1 / docs 0,无任何域净退,非单域运气。
- 终版(paths8)全景:139 = 93.5/99.3 @ 平均 72ms 最慢 218ms;895 = 61.6/90.9;回放词 89.5(1501,+2)/整句 85.4(−3,噪声带)。
  对局设计方的「边际带维持」意见系基于裸参数 +1.3pp 的过时快照;其自列复开条件(λ0.75 补扫)已执行并推至
  +1.4pp、139 同分变 +1.4,个人语域证据到位,裁决维持采纳。

| 尺 | 0.8B(λ0.5) | **2B(λ0.75)** | Δ |
|---|---|---|---|
| 895 主尺 | 60.2 / 90.5 | **61.6 / 90.9** | +1.4 / +0.4(p=0.035 裸参数口径) |
| 139 冻结 | 92.1 / 99.3 | **93.5 / 99.3** | +1.4 / 0.0 |
| 回放词 / 整句 | 89.3 / 86.0 | 89.5 / 85.4 | +3 / −3(噪声带) |
| 延迟(139 同步) | 63 ms | 72 ms | 最慢 218 ms,均在红线内 |
| 体积(裁后) | 672 MB | 1175 MB | +503 MB,app 1.3GB |

## 条件化 λ 的否定实验(2026-09-20,未合入)

按路线图第二阶段实现了规则式条件化 λ(静态前二分差 >3 nat 按比例降权、强个人证据 >6 nat 降权、
敲错路径 0.75 折),在两把尺上**都是净伤害**:139 尺 93.5→92.1(−1.4pp)、895 主尺 61.6→61.3。
规则里的「应该降权的情形」在数据上恰恰是神经该全额说话的情形(静态分差悬殊时神经修正本来就被 cap 压着,
再降 λ 等于双重惩罚;个人证据已有保护闸,叠加降权重复计账)。已全部撤回,工作区与 HEAD 均无残留。
**结论:条件化 λ 这条路要走训练式校准器(在分日/分域的交叉验证上拟合),规则式手拍因子无效。**
另:重排早退(静态领先 >2×cap 跳过打分)在单元测试里暴露语义问题——重排后的分数要喂跨切分仲裁,
不能只看池内翻不翻案,同样未合入。

## oracle 诊断后的召回现状(2026-09-20)

895 主尺 oracle@8=67.9%:32% 的正确句没进重排池,召回是上限。paths=16 只 +0.7(139),
双分歧组合(同一链上两处换次优前驱)是下一个真正的召回杠杆,但 diverged() 的构造是按单点分歧
写的,双点组合要在 backtrack 层重做,属于 viterbi 的结构性改动——留待专项(先写清验收:
oracle@8 提升幅度 + 139/895/回放三尺不退)。

## 第六轮:路线图推进(2026-09-20 下午)

按 `docs/下一步推进目标.txt` 逐项落地,以下是每项的状态与结论。

### 已落地(有代码、有验证、已推送)

| 路线图项 | 状态 | 数字/结论 |
|---|---|---|
| 一阶段1:oracle 指标 | ✅ `cce6224` | 139 oracle@8=95.0/top1 93.5(池不是瓶颈);**895 oracle@8=67.9——32% 正确句没进池,召回是上限** |
| 一阶段2:规范拼音泛化 | ✅ `ece33c9` | 选择按输入串+候选全拼双键,回放词 89.5→90.0、整句 85.4→87.1 |
| 一阶段3:换选负反馈 | ✅ `a428a53` | retract 时除退正分外记负分(独立 user-negatives.tsv),净数排序 |
| 一阶段4:句界上下文 | ✅ `b4231d8` | select_context 按句界装、垃圾段跳过;评测集量不出(上文都是自然语言),真实使用受益 |
| 评价体系:user effort | ✅ `3e3aa92` | 回放报告每千字换选/重打/翻页(4.0/29.6/0.0,共 8856 字) |
| 优先级六:日志挖词 | ✅ 验收修正 | mine_from_logs.py 完整实现换选/重打/深页/连续短语四类信号，并用产品词库过滤 OOV；仍需人工审核 |
| 优先级十:蒸馏数据 | ✅ 验收修正 | CLI 新增 `--eval-jsonl`，distill_dataset.py 真实调用 2B 导出正样本、神经重排 hard negatives 与 teacher rank；训练在训练仓库侧 |

### 否定实验(有数据、已撤、结论留档)

| 项 | 结论 |
|---|---|
| 二阶段6:规则式条件化 λ | **两尺净伤害**(139 −1.4pp/895 −0.3pp)——分差降权与个人证据降权在数据上恰恰砍在神经该说话的情形;要走训练式校准器 |
| 三阶段9(早退半):静态领先跳过打分 | 重排后分数要喂跨切分仲裁,不能只看池内翻不翻案,语义不成立 |

### 未动(有明确设计与验收口径,按优先级排)

| 项 | 现状与下一步 |
|---|---|
| 二阶段5:多样化 K-best/双分歧 | oracle 诊断已证明是最大杠杆;diverged() 是单点分歧构造,双点组合属 viterbi 结构改动,验收口径已写(oracle@8 提升+三尺不退) |
| 二阶段7:四域静态 LM | ✅ `245b9b6`——三域(lccc/wiki/zhihu)fill-in 子模型已打包(lm-lccc/lm-wiki/lm-zhihu.qj),CLI `--lm` 选择;139 混合 93.5 > zhihu 92.8 > lccc 89.9(评测集混域,分域价值在按应用选时体现) |
| 三阶段9:自适应 paths 扩展 | ❌ 否定实验 | 曾测得 139 93.5→94.2、895 61.6→61.9，但高歧义句最慢 320ms，越过预注册 300ms 一票否决线；已撤回，出厂固定 paths=8 |
| 三阶段11:学习时间衰减 | ✅ 验收修正 | user.tsv 第三列是「当前计数的基准日期」；加载衰减后刷新为今天，record 也刷新，避免每次重启对同一时段重复衰减 |

## 第七轮：短词功耗门控（2026-09-20）

真实日志 3892 次上屏里，两字上屏 2041 次，其中 1175 次标记为经过神经重排。这类输入通常是一两个音节的单词：
整句路径即使跑完 2B，也会在 `plain_sentence` 因「整段本来就是一个词」而与词级候选去重，属于高频无效前向。

出厂策略改为：少于 3 个音节时不生成 8 条重排路径、不唤醒 2B，仍由词级词频、个人选择与静态 LM 排序。
「功耗 gonghao」在补入基础词库后，已加载 2B 的单次查询从约 90 ms 降到 6.7 ms，首选仍是「功耗」。

## 第八轮：固定预算内的多样化召回（2026-09-20）

paths=16 能把 895 主尺 oracle 从 67.9% 抬到 68.9%，但让 2B 打 16 条已因 320ms 尾延迟被否决。
本轮只在便宜的 Viterbi 侧取 16 条：前 5 条按静态分数稳定保留，剩余 3 个名额优先覆盖相对首选的不同字符分歧组合，
再压回固定 8 条交给 2B。它能补到「三角…及数」这类“前缀与后缀同时分歧”的组合，但不增加神经前向条数。

| 尺 | 原 paths=8 | 多样化 16→8 | 变化 |
|---|---:|---:|---:|
| 139 首选 / 字准 / oracle | 93.5 / 99.3 / 95.0 | 93.5 / 99.3 / 95.0 | 持平 |
| 895 首选 / 字准 / oracle | 61.6 / 90.9 / 67.9 | **61.8 / 91.0 / 68.2** | +0.2 / +0.1 / +0.3pp |
| 139 平均 / 最慢 | 71.7 / 216ms | 71.5 / 219ms | 噪声带 |

## 第九轮：应用内偏好与后台完整预热（2026-09-20）

候选选择现在同时记全局键和 bundle id 隔离的应用键，排序使用“全局净选择 + 2 × 应用内净选择”。
因此代码编辑器与聊天工具可以对同一拼音形成相反偏好；新应用仍继承全局习惯。撤销记录保存上屏时的应用，
即使删词后切换了应用再重打，也只退回原应用的局部记录。单测覆盖相反偏好，workspace 294 个 Core 用例全绿。

此前 macOS 壳只用一个单字路径预热 2B，不能覆盖真实的 batch=8 张量形状；新 Metal 内核仍可能在第一条长句
懒编译，形成数秒尖峰。现在后台加载线程用 8 条不同长度的常见句一次预热，只有返回 8 个分数才接入引擎，
把编译成本移出用户首次输入。短词门控仍保证一、二音节完全不唤醒模型。

小型蒸馏 reranker 本轮不装入产品：仓库已有教师 JSONL 导出器，但训练仓库、训练产物和独立冻结验证集不在
当前工程。没有可复现产物时直接写一个启发式“小模型”会与 2B 重复打分，且无法证明收益；保留为离线实验门，
验收仍要求 139、895 与真实回放三尺不退，再讨论替换或级联 2B。

采纳：准确率与 oracle 同向上升，139 独立尺不退，2B 仍只打 8 条。

## 第十轮：上下文窗口改总量 200 自适应（2026-09-24）

`RESCORE_CONTEXT_CHARS` 128 → 200，语义从「前文上限」改成「光标前后**总预算**」：`split_window`
按两侧有无文本动态分——只有前文全给前文，只有后文全给后文，两侧都有对半（100/100），短侧用不完的让给对侧；
前文仍按句界结构化装填（`select_context`），后文取头部。壳读应用文本的长度跟着常量走（`RESCORE_LOOKBACK` = 200/200）。

A/B（同一份 600 行回放切片，冻结 `/tmp` 快照）：

| 窗口 | 词首选 | 词前五 | 整句首选 | 每千字换选/重打/翻页 |
|---|---:|---:|---:|---|
| 200（新） | 87.0%（255/293） | 97.3% | 77.3%（51/66） | 6.3 / 25.2 / 0.0 |
| 128（旧） | 87.4%（256/293） | 97.6% | 77.3%（51/66） | 6.3 / 25.2 / 0.0 |

差 1 个词（0.34pp，噪声带）、整句与用户努力度零差——**无回归、离线尺也量不出增益**。
原因诚实归因：评测与回放都只喂**左文**（`evaluate` 里 `history.record(pair.context)`，`after` 恒空），
128→200 只是左文变长，这一维在 895/139 上本来就不敏感；`split_window` 的价值在**右文**
（光标在句中编辑、后文存在的场景），只有真实使用能体现。

- 139 冻结尺：93.5 / 99.3（oracle 95.0，翻案转化 70.4%，翻坏 0.9%），平均 72.0 ms、最慢 225.4 ms（与 128 时代同带）。
- 895 主尺：61.8 / 91.0（oracle 68.2，翻案转化 23.8%，翻坏 2.8%），平均 103.7 ms。
  最慢 6792 ms 是**每进程首次 Metal 内核编译**（日志逐条 `compiling pipeline`；每次重编译二进制都会失效系统缓存），与窗口无关，装机 app 只加载一次。
- 单测：`split_window` 七种边界 + `rescoring_window` 的壳上下文/历史回退/右文取头（`engine/rescoring/{context,tests}.rs`）。

## 第十一轮：本地联想 / 同音纠错（2026-09-24）

联想框架复用云联想的 `Predictor` 管道（请求 / 序号 / Tab 接受 / 第一页末尾槽位），但由 Core 直接调本地 2B：
`RescoreWorker` 新增 `Predict` 任务类型（重排优先、同类只算最新、结果分渠道 `poll` / `poll_predict`），
`[predict] local`（缺省开，需 `[model] enabled`）接线。三类提议（都是「生成 + 2B 排序」，2B 不生成）：

1. **接续提议**：静态 LM 与个人 n-gram 的后继合并（个人计数折 8 次静态），胜过基线 guess 才出整句补全。
2. **深池词**：候选第一页（9 条）之外被静态排序埋没的词，与第一页参照比**每字平均分**（见下「长度偏置」）才进词槽。
3. **同音纠错**：最优转换路径（2–6 个词）的每个词用 `lookup_exact_all` 换同音词，静态整句分预筛取前 24 条；
   变体与 guess **等长**，与**基线**比、高出 `CORRECTION_MARGIN`(2.0 nat) 才进词槽。

门控：音节 < 3 不发（与短词门控同线）；上下文前后各 100 字，只进本机模型。

### 审查发现并修掉的三个问题

1. **评测污染（我引入）**：`request_prediction` 里的 `local_conversion` 会跑一次整句转换，
   而 `convert_sentence_with` 内部写 `rerank_probe` → 评测先跑纠错探针再读探针，读到的是纠错那次转换的池子，
   oracle 假跌 95.0 → 94.2。修法：`evaluate` 里先读探针、再跑纠错探针。修复后逐位回到 95.0 / 70.4 / 112。
2. **长度偏置（我引入）**：log 概率逐 token 累加，纠错变体（整句）与参照（混合长短的词候选）直接比分永远输
   （实测 -32.7 对 -10.9）。修法：纠错改与等长基线比；深池词改与参照比**每字平均分**（`per_char`）。
3. **退出崩（既有，被联想放大）**：`RescoreWorker` 原先不等线程退完（注释写「模型可能正算到一半」），
   进程退出时 ggml 撞 `GGML_ASSERT([rsets->data count] == 0)` 直接 abort。修法：`Drop` 里先关任务通道再 `join`
   （最多等手上一条推理）。对照验证：修前异步评测必崩、修后干净退出。

### 装机实测暴露并修掉的两个问题（2026-09-24 晚）

1. **每键都发联想请求 → 把重排挤超时**：本地联想原先跟云联想不同，壳里**没有防抖**，`refresh` 每键都发。
   一次批次 ~80 条（基线 + 24 接续 + 9 参照 + 30 深池 + 24 纠错）× 带上下文的 2~3 遍前向，
   后台线程被占住；日志实测出现 `texts=408 ms=5147`（重排的「待打分」集合在整段组句里累积）与
   `等本地整句模型超时，本轮不重排`——用户直接看到「没有联想」。
   修法：**壳加 250ms 防抖**（`PredictMonitor::schedule` → `Host::start_prediction`，复用停键时机）
   + **批次砍到 ~30 条**（接续 8 / 参照 5 / 深池 8 / 纠错 8）。实测单批 399 ms。
2. **壳只传了第一页 9 条候选**：深池从第 10 条起算，于是**装机环境里深池恒为空**（CLI 传全池所以评测看不出来）。
   修法：`Host::start_prediction` 重新 `engine.query()` 拿完整候选列表再发。
   顺带：没有应用前文时本地任务退回本会话历史（与重排一致）。

另外没有应用前文（空文档、Electron、终端）时 `has_context=false`，纠错会明显变弱——空文档读不到前文是预期行为。

### 装机端到端确认（TextEdit，2026-09-24）

上下文「我最近在学画画，」+ 输入 `tadehuazaimeishuguanzhanchu`：
`guess=他的话在美术馆展出`（静态首选错）→ 纠错槽 `words=["他的画在美术馆展出"]` → `云端词已补进候选 filled=1`，
候选窗口第 9 格可见（带云朵标识，见下）。**第一阶段端到端成立**。

重排「待打分」集合的累积已收口：新增 `RESCORE_BATCH_MAX` = 32（`NeuralCache::take_wanted(limit)` 取最近攒的至多 32 条，
其余留到下一批；当前查询的路径最后压入，一定在这批里）。装机实测 `texts=408 ms=4513` → `texts=32 ms=312`，不再挤穿壳的 2 秒等待。

本地联想的结果改标 `CandidateKind::Local`（行为与 `Cloud` 相同、候选窗不画云朵）：装机截图确认纠错候选
`他的画在美术馆展出` 不再带 ☁ 标识。

### 尺子（`--local-prediction` + `--neural-async`，报告加「本地联想/纠错」一行）

| 尺 | 纠错槽含原句 | 救回（首选错却被捞回） | 首选已对却有纠错 | 重排三尺 |
|---|---:|---:|---:|---|
| 139 冻结 | 1.4%（2/139） | 7.4%（2/27） | 0.9%（1/112） | 93.5 / 95.0 / 70.4 不变 |
| 895 主尺 | 1.9%（17/895） | 3.9%（17/432） | 0.4%（2/463） | 61.8 / 68.2 不变 |

冒烟样例（top1 错、纠错槽给对）：`他的画在美术馆展出`（top1 他的话）、`请你不要再犹豫了`（top1 不要在）。

**口径限制（诚实说）**：现在的指标只看「纠错槽里有没有原句」，把**生成召回**与 **2B 的选择**混在一起；
还没有 `oracle@wide`（原句在生成变体集里的比例）这一层。另外纠错只做**单点替换**，
双处同时分歧（文档里的老难题）仍要动 viterbi；两个音节的输入（日志里多数 miss）仍被短词门控挡着。
