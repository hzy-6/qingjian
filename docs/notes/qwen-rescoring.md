...
| 二阶段7:四域静态 LM | 语料已就位(wiki 27M 行/lccc 20M/zhihu 53 万);第三轮的教训(替换必损回放、fill-in 是正解)直接适用——四域 fill-in 而非四域替换,域检测用应用类型配置 |
| 三阶段9(主):自适应 paths 扩展 | 早退半已证伪;扩展半(不确定时升到 12/16)依赖重跑 convert,与查询流程耦合紧,留待多样化 K-best 一起做 |
| 三阶段11:学习时间衰减 | user.tsv 无时间戳,加列是格式变更;明确添加的用户词不衰减的要求要先理清 user-words 与自动学习的存储分层 |
