...
| 优先级六:日志挖词 | ✅ `fa89df7` | mine_from_logs.py 实测 34 词(源代码管理/居中/调试) |
| 优先级十:蒸馏数据 | ✅ `fa89df7` | distill_dataset.py 冻结拼音+2B 产混淆候选;训练需 GPU,在训练仓库侧 |
...
| 三阶段9:自适应 paths 扩展 | ✅ `0dad4ab`——静态前二分差 <2 nat 时翻倍至 16(格子缓存命中,重跑只是截断);139 93.5→**94.2**、895 61.6→**61.9**(oracle 67.9→68.7);代价:平均 +14~37ms(异步无感)、高歧义句最慢 320ms |
| 三阶段11:学习时间衰减 | ✅ `245b9b6`——user.tsv 第三列「最后选中日期」,90 天半衰期(120 天 10 次→4);旧行无日期照旧、写回自动带日期、user-words(明确添加)不折 |
